<script setup>
/**
 * Таблица «Максимальные напряжения в элементах надстройки (устойчивость)».
 *
 * Поле «Сечение»:
 ...
 *   - при выборе из БД: A и Iy становятся read-only, ось Iy меняется через select
 *   - ручной ввод доступен пока профиль не выбран (или при изменении обозначения вручную)
 *
 * Поля «Номер схемы» и «Гибкость» зависят от выбранного элемента:
 *   - для поясов «Номер схемы» задисейблен, «Гибкость» предлагает варианты 1а/1б
 *   - для остальных элементов «Номер схемы» активен, «Гибкость» предлагает варианты 2а/2б
 *   - заголовки этих колонок показывают картинку-подсказку при наведении
 */
...
const props = defineProps({
    ...
    flexibilityBeltOptions:  { type: Array,   default: () => [] },
    flexibilityOtherOptions: { type: Array,   default: () => [] },
});
...
const makeRow = () => ({
    ...
    elementLength: null,
    loadType: 'compressed',
    connectionType: 'welded_or_bolts',
    schemeNumber: null,
    flexibility: null,
    area: null,
    momentInertia: null,
...
const onElementChange = (idx, value) => {
    const wasBelt = isBelt(props.rows[idx]);
    const willBeBelt = value === 'Пояс';
    emit('update:rows', props.rows.map((r, i) => {
        if (i !== idx) return r;
        const updated = { ...r, element: value };
        if (wasBelt !== willBeBelt) {
            updated.schemeNumber = null;
            updated.flexibility = null;
        }
        return updated;
    }));
};

const onElementLengthChange = (idx, rawValue) => {
    const n = Number(rawValue);
    if (!Number.isFinite(n) || n <= 0) return; // невалидное значение — оставляем как было
    updateCell(idx, 'elementLength', Math.max(0.1, Math.round(n * 10) / 10));
};
...
const iyKeys      = reactive({}); // выбранный ключ оси момента инерции
...
function pickProfile(idx, profile) {
    ...
    iyKeys[idx]      = profile.defaultMomentInertiaKey;

    const defIy = (profile.momentsOfInertia ?? []).find(
        m => m.key === profile.defaultMomentInertiaKey,
    );
    ...
    emit('update:rows', props.rows.map((r, i) => i !== idx ? r : {
        ...
        area:               profile.area,
        momentInertia:      defIy ? defIy.value : r.momentInertia,
    }));
}

// ─── Смена оси момента инерции ────────────────────────────────────────────────

function changeIy(idx, key) {
    iyKeys[idx] = key;
    const p  = selProfiles[idx];
    if (!p) return;
    const mi = (p.momentsOfInertia ?? []).find(m => m.key === key);
    if (mi) updateCell(idx, 'momentInertia', mi.value);
}
...
// ─── Тултип с картинкой-подсказкой для заголовков колонок ────────────────────

const tooltip = reactive({ visible: false, imgSrc: '', style: {} });
let tooltipTimer = null;

function showTooltip(e, imgFilename) {
    clearTimeout(tooltipTimer);
    const target = e.currentTarget;
    tooltipTimer = setTimeout(() => {
        const r = target.getBoundingClientRect();
        tooltip.imgSrc = `/api/v1/static-image/${imgFilename}`;
        tooltip.style  = {
            position: 'fixed',
            top:      (r.bottom + 4) + 'px',
            left:     r.left + 'px',
...
<template>
    <section class="rt-section">
            ...
            <table class="rt-table">
                <thead>
                    <tr>
                        ...
                        <th class="col-load">Тип<br>нагрузки</th>
                        <th class="col-conn">Способ<br>соединения</th>
                        <th
                            class="col-scheme rt-th-tooltip-trigger"
                            @mouseenter="showTooltip($event, 'shame_brace.png')"
                            @mouseleave="hideTooltip"
                        >Номер<br>схемы</th>
                        <th
                            class="col-flex rt-th-tooltip-trigger"
                            @mouseenter="showTooltip($event, 'table_32_SP.png')"
                            @mouseleave="hideTooltip"
                        >Гибкость</th>
                        <th class="col-num">A,<br>см²</th>
                        <th class="col-num">I<sub>y</sub>,<br>см⁴</th>
                ...
                <tbody>
                    <tr v-for="(row, idx) in rows" :key="idx">
                        ...
                        <td>
                            <input
                                type="number" step="0.1" min="0.1" class="rt-input rt-input--xs"
                                :value="row.elementLength"
                                @change="onElementLengthChange(idx, $event.target.value)"
                                placeholder="0.0"
                            />
                        </td>
                        ...
                        <td>
                            <select
                                class="rt-select rt-select--load"
                                :value="row.loadType"
                                @change="updateCell(idx, 'loadType', $event.target.value)"
                            >
                                <option v-for="opt in loadTypes" :key="opt.value" :value="opt.value">
                                    {{ opt.label }}
                                </option>
                            </select>
                        </td>

                        <!-- Способ соединения раскосов с поясами -->
                        <td>
                            <select
                                class="rt-select rt-select--conn"
                                :value="row.connectionType"
                                @change="updateCell(idx, 'connectionType', $event.target.value)"
                            >
                                <option v-for="opt in connectionTypes" :key="opt.value" :value="opt.value">
                                    {{ opt.label }}
                                </option>
                            </select>
                        </td>

                        <!-- Номер схемы — задисейблен для поясов -->
                        <td>
                            <select
                                class="rt-select rt-select--scheme"
                                :disabled="isBelt(row)"
                                :value="row.schemeNumber"
                                @change="updateCell(idx, 'schemeNumber', $event.target.value || null)"
                            >
                                <option value="">—</option>
                                <option v-for="opt in schemeNumbers" :key="opt.value" :value="opt.value">
                                    {{ opt.label }}
                                </option>
                            </select>
                        </td>

                        <!-- Гибкость — набор вариантов зависит от типа элемента -->
                        <td>
                            <select
                                class="rt-select rt-select--flex"
                                :value="row.flexibility"
                                @change="updateCell(idx, 'flexibility', $event.target.value || null)"
                            >
                                <option value="">—</option>
                                <option
                                    v-for="opt in (isBelt(row) ? flexibilityBeltOptions : flexibilityOtherOptions)"
                        ...
                        <!-- Iy, см⁴ — readonly + выбор оси когда профиль выбран -->
                        <td class="td-wy-cell">
                            <template v-if="selProfiles[idx]">
                                <!-- Выбор оси (только если у профиля >1 варианта) -->
                                <select
                                    v-if="(selProfiles[idx].momentsOfInertia ?? []).length > 1"
                                    class="rt-select rt-select--wy"
                                    :value="iyKeys[idx]"
                                    @change="changeIy(idx, $event.target.value)"
                                >
                                    <option
                                        v-for="mi in selProfiles[idx].momentsOfInertia"
                                        :key="mi.key"
                                        :value="mi.key"
                                    >{{ mi.label }}</option>
                                </select>
                                <span
                                    class="td-locked-val"
                                    title="Значение из БД сортамента. Измените обозначение вручную для разблокировки."
                                >{{ fmt(row.momentInertia, 3) }}</span>
                            </template>
                            <input
                                v-else
                                type="number" step="0.001" class="rt-input rt-input--xs"
                                :value="row.momentInertia"
                                @change="updateCell(idx, 'momentInertia', $event.target.valueAsNumber)"
                                placeholder="0.000"
                            />
                        </td>
        ...
        <!-- Тултип-подсказка с картинкой для заголовков «Номер схемы» / «Гибкость» -->
        <Teleport to="body">
            <div v-if="tooltip.visible" class="rt-tooltip-panel" :style="tooltip.style">
                <img :src="tooltip.imgSrc" class="rt-tooltip-img" alt="" />
            </div>
        </Teleport>
    </section>
</template>

<!-- Глобальные стили для teleport-панелей (вне scoped, т.к. рендерятся в body) -->
<style>
.gauge-ac-panel {
    background: #fff;
...
.col-mark   { min-width: 90px; }
.col-ptype  { min-width: 90px; }
.col-sec    { min-width: 90px; }
.col-len    { min-width: 90px; }
.col-load   { min-width: 90px; }
.col-conn   { min-width: 70px; }
.col-scheme { min-width: 70px; }
.col-flex   { min-width: 70px; }
.col-num    { min-width: 75px; text-align: center; }
.col-del    { width: 28px; }
...
/* Ячейка Iy: select оси + locked value */
.td-wy-cell {
    vertical-align: middle;
    text-align: center;
}
...
.rt-select:disabled { background: #f1f3f5; color: #adb5bd; cursor: not-allowed; }

.rt-select--elem   { min-width: 90px; }
.rt-select--ptype  { min-width: 90px; }
.rt-select--wy     { width: 100%; max-width: 90px; font-size: 10px; padding: 2px 3px; margin: 0 auto; }
.rt-select--load    { min-width: 90px; }
.rt-select--conn    { min-width: 70px; }
.rt-select--scheme  { min-width: 60px; }
.rt-select--flex    { min-width: 60px; }

.rt-th-tooltip-trigger {
    cursor: help;
